...
public class ReviewProvider {
    ...
    //리뷰 리스트 조회
    public List<GetReviewRes> getReview(int store_idx) throws BaseException {
        //store 존재여부 확인
        ...
        try{
            List<GetReviewRes> getReview = reviewDao.getReview(store_idx);
            return getReview;
        } catch(Exception exception){
    ...
    //상태별 리뷰 리스트 조회
    public List<GetReviewTypeRes> getReviewType(int store_idx,String type) throws BaseException {
        //store 존재여부 확인
        if(checkStoreExist(store_idx) ==0){
            throw new BaseException(STORE_EMPTY_STORE_IDX);
        }
        ...
        try{
            List<GetReviewTypeRes> getReviewType = reviewDao.getReviewType(store_idx,type);
