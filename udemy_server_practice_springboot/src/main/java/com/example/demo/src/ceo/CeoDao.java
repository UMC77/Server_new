...
public class CeoDao {
    ...
    public int createCeo(PostCeoReq postCeoReq){
        String createCeoQuery = "insert into ceo (store_num,ceo_id, ceo_pwd, ceo_phone, ceo_name) VALUES (?,?,?,?,?)";
        Object[] createCeoParams = new Object[]{postCeoReq.getStore_num(),postCeoReq.getCeo_id(),postCeoReq.getCeo_pwd(), postCeoReq.getCeo_phone(), postCeoReq.getCeo_name()};
    ...
    public int createStore(int ceoIdx, PostStoreReq postStoreReq){
        String createStoreQuery = "insert into store (store_ceo_idx, store_name, store_phone, store_address, store_info) VALUES (?,?,?,?,?)";
        Object[] createStoreParams = new Object[]{ceoIdx, postStoreReq.getStore_name(), postStoreReq.getStore_phone(), postStoreReq.getStore_address(), postStoreReq.getStore_info()};
    ...
    public int checkStoreNum(String store_num){
        String checkStoreNumQuery = "select exists(select store_num from ceo where store_num = ?)";
